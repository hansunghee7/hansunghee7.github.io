...
> 중복 행을 정리. **다섯 번째 정리(2026-08-31, 사장님 직접 지시)**: `shorts-lab`
> 분리가 8/30 저녁 한 번 보류됐었으나, 그 사이 ux-lead가 겸임하며 실제 정리
> 건을 완료시킨 걸 확인하고 오늘 정식으로 분리 — [숏감독.md](숏감독.md) 신설,
> ux-lead는 shorts-lab 겸임 종료.
...
| [ux-lead.md](ux-lead.md) | **공개 사이트+스튜디오 UX 총괄**(튜터명 "시안"/"노트" — 세션마다 다르게 불림, 아직 안 맞춰짐) — 너비 정책, 인사이트 카드/그래프 표준, 스튜디오 사이드바, 스튜디오 UX 거버넌스(STYLE_GUIDE.md/ⓘ 패턴/studio.css·js, 2026-08-30 simplifier-studio에서 이관). **`shorts-lab` 겸임은 2026-08-31 숏감독 분리로 종료** | 표준 수립 |
...
| [숏감독.md](숏감독.md) | `shorts-lab` 파이프라인(저장소 밖) + 스튜디오 숏폼 정본 문서 5개 전담. 2026-08-30 한때 보류됐다가 2026-08-31 정식 분리 확정 | 실무(코드) |
...
- ✅ (완료) `log.html` 무한 스크롤 → "더 보기" 버튼 방식 전환 — 사장님이
  직접 승인해 `ux-lead`가 진행(PR #37, 2026-08-30). 자동 IntersectionObserver/
  scroll 트리거를 제거하고 클릭 시에만 다음 20개를 불러오도록 바꿔, 스크롤만
  해도 끝없이 늘어나던 문제(전체 펼치면 약 49,000px)를 없앴다. 카드 587개를
  전부 정적 HTML로 렌더링해두는 근본 구조(447KB 페이지 무게)는 이번 범위 밖 —
  트래픽·지표상 필요해지면 별도로 재검토. **부수 발견**: 카테고리 칩(`.cat-btn`)
  클릭은 실제로는 필터링이 아니라 해당 필러 페이지로의 진짜 네비게이션이라(JS의
  필터 로직에 `preventDefault()`가 없음), 화면상 필터처럼 보이지만 실은 페이지
  이동이다 — 결과가 같아서 눈에 띄는 버그는 아니지만 죽은 코드에 가깝다. 급하지
  않아 손대지 않았고, site-repo-ops 쪽에 공유용으로 남겨둠.
- ✅ (완료) `shorts-lab` 생성기 정리 — "숏감독" 스레드로 이관됐었으나, `ux-lead`가
  착수 전 이 표를 못 보고 사장님 대화창 지시만 믿고 진행·완료함(`shorts-lab@de6099a`
  + PR #33, 2026-08-30). **같은 날 저녁 사장님이 스레드 분리 자체를 보류**시켰다 —
  `.wrap{max-width:760px}` 죽은 규칙 제거 + 정본 문서 5개 재생성 건은 끝났지만,
  "숏감독"을 새로 만드는 건 미루고 **당분간 `shorts-lab` 관련 요청은 `ux-lead`가
  계속 받는다**("잘 굴러가는 게 확인되면" 그때 분리). 위 스레드 목록 표도 갱신.
  **후속(2026-08-31): 정식 분리 확정** — [숏감독.md](숏감독.md) 신설, ux-lead의
  shorts-lab 겸임 종료. 위 스레드 목록 표도 다시 갱신.
