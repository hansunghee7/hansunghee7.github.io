...
| [ux-lead.md](ux-lead.md) | **공개 사이트+스튜디오 UX 총괄**(튜터명 "시안"/"노트" — 세션마다 다르게 불림, 아직 안 맞춰짐) — 너비 정책, 인사이트 카드/그래프 표준, 스튜디오 사이드바, 스튜디오 UX 거버넌스(STYLE_GUIDE.md/ⓘ 패턴/studio.css·js, 2026-08-30 simplifier-studio에서 이관). **`shorts-lab` 파이프라인도 "숏감독" 분리 전까지 당분간 겸임(2026-08-30)** | 표준 수립 |
...
| 숏감독 (아직 없음, 분리 보류) | `shorts-lab` 파이프라인 등 숏폼 제작 인프라 전담으로 2026-08-30 한때 신설 지시됐으나, 같은 날 저녁 **분리 자체를 보류**(사장님 지시) — "잘 굴러가는 게 확인되면" 그때 만든다. 그때까지 관련 작업은 `ux-lead`가 들고 있음 | 실무(코드), 보류 |
