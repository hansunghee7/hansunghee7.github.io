...
- 동작(hover 데스크톱 / 탭 토글 터치기기, 하나 열면 나머지 자동 닫힘)은
  ...
  핸들러를 만들지 않습니다.
- **화면 오른쪽 끝 근처의 ⓘ는 열릴 때 뷰포트 밖으로 넘치지 않게 자동으로
  뒤집힙니다**(`studio.js`의 `positionInfoText()` — hover/클릭 직전에
  `getBoundingClientRect()`로 넘칠지 계산해 `.info-text--right` 클래스로
  왼쪽 대신 오른쪽 기준 앵커로 전환). 좁은 창에서 가로 스크롤이 생기던 버그를
  2026-08-30에 고쳤습니다 — 아래 "과거에 실제로 있었던 사고" 참고. `.h2note-text`
  (사이트 인사이트 전용, 가운데 정렬 변형)는 같은 함수가 `--tt-shift` CSS 변수로
...
- **`title=` 죽은 툴팁**: book-insight/sns-insight의 초기 버전, 그리고 외부
  ...
  패턴으로 교체.
- **ⓘ 말풍선이 화면 오른쪽 밖으로 넘침**: `.info-text`가 항상 `left:0`
  기준으로 오른쪽으로 펼쳐지는 구조라, 화면 오른쪽 끝 근처의 ⓘ를 좁은 창에서
  열면 말풍선이 뷰포트 밖으로 나가 가로 스크롤이 생겼음(CMO 스레드가
  발견해 작업 칩으로만 등록해두고 미착수 상태였던 버그, 2026-08-30 ux-lead
  스레드가 확인 후 수정). 뷰포트 경계를 감지하는 로직 자체가 아예 없었던 게
  원인 — 고칠 때 `positionInfoText()`로 일반화해 `.h2note-text` 변형도 같이
  대응하게 함.
