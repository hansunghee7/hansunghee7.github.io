...
def collect_post_views(browser, log_no):
    ...
    try:
        ...
        # 2026-09-02: "조회수"가 매번 0/2로 실패해 진단 로그를 남겼더니
        # (스크롤 후 재시도까지 해봐도 동일), 원인은 지연 로딩이 아니라
        # 이 채널 게시글이 전부 "동영상" 포스트라 조회수 대신 영상 위젯의
        # "재생 N"(+"좋아요 N")을 쓴다는 것이었다 -- 실제 진단 스니펫에
        # "재생 1 좋아요 0"이 그대로 보임. 일반 글(사진/텍스트)이 섞이면
        # 그쪽은 "조회수"를 쓸 수 있어 둘 다 시도한다.
