...
## 참고: 구글 색인 현황 (2026-09-02 점검)

`scripts/fetch_index_coverage.py`(주 1회 자동 실행) 결과, 글 587편 중 **144편만
색인됨(24.5%)**. `canonical_mismatch` 0건 — 구글이 네이버를 원본으로 재배정한
사례가 없으므로, **"네이버와 중복돼서 구글이 안 실어준다"는 가설은 틀렸다**(2026-09-02
진단 세션에서 처음 낸 가설을 데이터로 정정). 카테고리·연도별 색인율이 22~29%로
고르게 낮아, 특정 글의 문제가 아니라 **사이트 전체 권위(백링크) 부족에 따른 크롤
예산 문제**로 보인다. 재유통이 SNS에서 클릭 신호를 만들면 색인율에도 간접 도움이
될 가능성이 있지만 확정된 인과관계는 아니다 — 색인 자체를 목표로 뭘 더 하지 말고,
`assets/data/index-coverage.json`이 주 1회 갱신되니 재유통을 몇 주 진행한 뒤
`posts_summary.indexed` 값이 움직이는지만 곁눈질로 확인한다.
